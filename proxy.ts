...
const RATE_LIMIT_RULES: Record<string, { maxRequests: number; windowMs: number }> = {
  '/api/whatsapp/webhook': { maxRequests: 20, windowMs: 60_000 },
  '/api/whatsapp/test':    { maxRequests: 10, windowMs: 60_000 },
  '/api/cron':             { maxRequests: 5,  windowMs: 60_000 },
}
...
function isRateLimited(key: string, rule: { maxRequests: number; windowMs: number }): boolean {
  ...
  if (entry.count >= rule.maxRequests) return true
  entry.count++
  return false
}
// ─────────────────────────────────────────────────────────────────────────────
...
export async function proxy(req: NextRequest) {
  ...
  if (rule) {
    ...
    if (isRateLimited(key, rule)) {
      console.warn('[proxy] rate-limit-exceeded', { ip, pathname })
